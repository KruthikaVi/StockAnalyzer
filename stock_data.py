...
# delete stocks and daily data from database
def delete_stock_data(symbol):
    stockDB = "stocks.db"
    conn = sqlite3.connect(stockDB)
    cur = conn.cursor()
    deleteStockCmd = """DELETE FROM stocks WHERE symbol = ?; """
    deleteDailyDataCmd = """DELETE FROM dailyData WHERE symbol = ?;"""
    try:
        cur.execute(deleteStockCmd, (symbol,))
        conn.commit()
        cur.execute(deleteDailyDataCmd, (symbol,))
        conn.commit()
    except Exception as e:
        print(f"Error in deleting the stock {str(e)}")
